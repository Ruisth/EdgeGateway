...
# 2. Create the local secrets file and replace every placeholder
cp .env.example .env
$EDITOR .env

# 3. Generate TLS certificates for MQTT and the Ditto basic-auth file
cd services/mosquitto/certs && bash generate-certs.sh && cd -
bash services/ditto/nginx/generate-htpasswd.sh

# 4. Start all services (~20 containers)
docker compose up -d

# 5. Verify the EGW Controller is healthy
curl http://localhost:8090/health
# Expected: {"status": "ok", "service": "egw-controller"}

# 6. Run the full lifecycle demo (UC1–UC8)
python services/egw-controller/examples/run_full_lifecycle.py
```

> The `.env` file holds every credential consumed by Compose (Ditto, CouchDB,
> ACA-Py wallet keys). It is gitignored. Compose fails fast if any required
> variable is missing.
